...
8. 对已经运行的转发线路，管理员可按需排队一次互通诊断。入口 Agent 只读取本机已安装的转发资源，向其中记录的落地地址和端口建立一次 TCP 连接，最多等待 3 秒。诊断结果独立于部署状态；每条线路有 60 秒冷却时间，不自动周期探测，旧记录沿用任务队列的七天清理策略。入口 Agent 需更新至 0.6.9 或更高版本。
...
互通诊断的毫秒数为入口机 DNS 解析及 TCP 建连时间，不是 ICMP ping，也不包含出口代理认证、实际代理浏览、带宽或客户端至入口的延迟。TCP 可连仍应完成客户端订阅和协议实测；若失败，先检查落地服务监听、端口防火墙和 DNS。
