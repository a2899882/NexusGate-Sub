...
转发线路部署完成后，“转发与节点”页面可点击“测试互通”，分别查看每台入口到落地端口的 TCP 建连结果和时间。先在入口机执行 `ng-agent update` 升级到 0.6.9；无需在落地机额外安装测试组件。诊断按需运行，不会定时消耗资源，单条线路至少间隔 60 秒，最多同时排队 50 项、单次最多 20 台入口。它不验证代理认证、HY2 的 UDP 客户端入口、真实浏览流量或带宽；失败不更改运行中的线路状态。

0.6.10 起，线路列表直接显示单入口延迟或多入口延迟范围，可按名称、节点、客户与诊断状态筛选，每页 25 条；勾选后批量测试最多 25 条线路、30 个入口任务，同一入口最多积压 8 个诊断任务。不可测试的线路会逐条说明原因。服务器在线状态、线路部署状态和诊断结果在页面可见时自动检查变化；排队后约每 5 秒检查一次，其余时间约每 15 秒检查一次，离开页面或打开对话框时暂停。自动刷新不会重置搜索、选择或已展开的详情。

0.6.11 将线路勾选框放在独立窄列，压缩搜索和批量按钮、行间距。诊断列只显示一个结果：130 ms 以下为绿色，130 ms 及以上为黄色，失败显示红色“不可达”，排队中显示“测试中”；点结果可查看逐入口原因和时间。多入口显示延迟范围及成功数；多台设备和多个客户在列表中折叠显示数量，悬停可看完整名称。
